...
function OpenATCCDialog() {
    $("#ATCC-Dialog-Box").dialog(opt).dialog("open");
}

function GetAtccDetail() {
    ...
    var atccdetails = atcces;
    var tot_rec = atcces.length;
    ...
    if (tot_rec > 0) {
        ...
        $("#ItemAtccDetail").find("tr").remove();

        // Display All Atcc devices on the screen
        $.each(atccdetails, function (i, d) {
            var atmsId = d.atmsId;
            ...
            if (cur_col <= col) {
                //cstr += '<td><div id="div_atcc' + deviceId + '" class="atcc_data"><div id="' + deviceId + '">' + ipAddress + '</div><div id="' + deviceName + '">' + deviceName + '</div><div><img id=imgAtcc'+ deviceId +' src=' + imgsrc + '></img></div></div></td>';
                cstr += '<td><div id="div_atcc' + deviceId + '" class="atcc_data" style="background-color:#ff0000;">';
                cstr += '<div style="width:20%; float:left; min-height:95px;">';
                cstr += '<img id=imgAtcc' + deviceId + ' src=' + imgsrc + ' class="position: relative;top: 50%;"></img>';
                cstr += '</div>';
                ...
                cstr += '<div>Status:<span id="spStatusAtcc' + deviceId + '"></span></div>';
                cstr += '</div>';
                cstr += '</div></td>';
                ++item_added;
                ...
                if (item_added == tot_rec) {
                    if (cur_col < col) {
                        ...
                        if (left_col > 0) {
                            ...
                            $(complete_row).appendTo('#ItemAtccDetail');
                            cur_col = 1;
                            ++cur_row;
                            rstr = '';
                            cstr = '';
        ...
        // Show the live status of Atcc devices
        Get_Atcc_live_status();
    }
}

// Show live status of Atcc devices
function Get_Atcc_live_status() {
    if (atcces_live != null && atcces_live.length > 0) {
        $.each(atcces_live, function (i, d) {
            var atmsId = d.atmsId;
            var chainage = d.chainage;
            var controlRoomID = d.controlRoomID;
            var deviceCategoryId = d.deviceCategoryId;
            ...
            if (deviceStatus == 0) {
                $('#div_atcc' + deviceId).attr('style', "" + "background-color:#ff0000; border-color: #ff0000;" + "");
                imgsrc = deviceInactiveImagePath;
                strStatus = 'Inactive';
            }
            else if(deviceStatus == 1)
            {
                $('#div_atcc' + deviceId).attr('style', "" + "background-color:#00CE67; border-color: #00CE67;" + "");
                imgsrc = deviceActiveImagePath;
                strStatus = 'Active';
            }

            $('#spStatusAtcc' + deviceId).text(strStatus);
            // change the image sourc as active or In active
            $('#imgAtcc' + deviceId).attr('src', imgsrc);
        });
    }

    // Set 15 seconds interval
    setTimeout(Get_Atcc_live_status, 15000);
}