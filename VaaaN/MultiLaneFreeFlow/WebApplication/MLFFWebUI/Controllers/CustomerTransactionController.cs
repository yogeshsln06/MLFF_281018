            ...
            else
            {
                try
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strfilter = " WHERE TRANSACTION_DATETIME BETWEEN TO_DATE('" + objtransaction.TransactionDateTime.AddSeconds(-Seconds).ToString("dd/MM/yyyy HH:mm:ss") + "','DD/MM/YYYY HH24:MI:SS') AND TO_DATE('" + objtransaction.TransactionDateTime.AddSeconds(Seconds).ToString("dd/MM/yyyy HH:mm:ss") + "','DD/MM/YYYY HH24:MI:SS') AND T.AUDITED_VRN='" + AuditedVRN + "' AND NVL(T.AUDIT_STATUS,0)=1 AND NVL(T.MEARGED_TRAN_ID,0)=0 AND T.PLAZA_ID=" + objtransaction.PlazaId + "";
                        // shiv changed AddMinutes to AddSeconds in between TO_date
