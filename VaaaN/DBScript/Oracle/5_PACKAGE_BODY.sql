...
SELECT * FROM TBL_TRANSACTION WHERE TRANSACTION_DATETIME >= P_START_TIME AND TRANSACTION_DATETIME <= P_END_TIME AND IS_REGISTERED = 1
...
SELECT TRAN.TRANSACTION_DATETIME AS TIME_STAMP,
  ...
  FROM CTE TRAN
       ...
       LEFT OUTER JOIN TBL_VEHICLE_CLASS VC1
          ...
          WHERE NVL(TRAN.CT_ENTRY_ID,0)>0
         ...
         --Crostalk packet not found but nf front packet found
  ...
  FROM CTE TRAN
       ...
       LEFT OUTER JOIN TBL_CUSTOMER_VEHICLE CUST_VEH
          ...
          ON UPPER(NFP.PLATE_NUMBER) = UPPER(CUST_VEH.VEH_REG_NO)
       ...
       LEFT OUTER JOIN TBL_VEHICLE_CLASS VC1
          ...
          WHERE NVL(TRAN.CT_ENTRY_ID,0)<=0 AND NVL(TRAN.NF_ENTRY_ID_FRONT,0) >0;
...
TRANSACTION_DATETIME >= P_START_TIME AND TRANSACTION_DATETIME <= P_END_TIME AND IS_REGISTERED = 2
